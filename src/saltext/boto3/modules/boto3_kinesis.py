...
=================================================
    ...
    Renamed from ``boto_kinesis`` to ``boto3_kinesis`` and rewritten to use the
    boto3 ``kinesis`` client APIs directly via
...
:configuration: This module accepts explicit kinesis credentials but can
...
def __virtual__():
    ...
    return (False, "The boto3_kinesis module could not be loaded: boto3 is not available.")
...
def _get_basic_stream(stream_name, conn):
    ...
    .. code-block:: bash
        ...
        salt myminion boto3_kinesis._get_basic_stream my_stream existing_conn
    """
...
def _get_full_stream(stream_name, region=None, key=None, keyid=None, profile=None):
    ...
    .. code-block:: bash
        ...
        salt myminion boto3_kinesis._get_full_stream my_stream region=us-east-1
    """
    conn = _get_conn("kinesis", region=region, key=key, keyid=keyid, profile=profile)
    r = {}
...
def reshard(
    ...
    for shard_num, shard in enumerate(stream_details["OpenShards"]):
        ...
        expected_ending_hash_key = min(expected_ending_hash_key, max_hash_key)
...
def _get_next_open_shard(stream_details, shard_id):
    ...
    .. code-block:: bash
        ...
        salt myminion boto3_kinesis._get_next_open_shard existing_stream_details shard_id
    """
    found = False
...
def _execute_with_retries(conn, function, **kwargs):
    ...
    **kwargs
    ...
    .. code-block:: bash
        ...
        salt myminion boto3_kinesis._execute_with_retries existing_conn function_name function_kwargs
...
def _jittered_backoff(attempt, max_retry_delay):
    ...
    .. code-block:: bash
        ...
        salt myminion boto3_kinesis._jittered_backoff current_attempt_number max_delay_in_seconds
    """
