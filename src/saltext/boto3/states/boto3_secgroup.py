"""
Manage Security Groups using boto3.
===================================

    Renamed from ``boto_secgroup`` to ``boto3_secgroup`` and updated to call the
    refactored ``boto3_secgroup`` execution module.
...
def __virtual__():
    ...
    """
    if "boto3_secgroup.exists" in __salt__:
        return "boto3_secgroup"
    return (False, "boto3_secgroup module could not be loaded")
...
def _security_group_present(
    ...
    exists = __salt__["boto3_secgroup.exists"](name, region, key, keyid, profile, vpc_id, vpc_name)
    if not exists:
        if __opts__["test"]:
            ret["comment"] = f"Security group {name} is set to be created."
            ret["result"] = None
            return ret
        created = __salt__["boto3_secgroup.create"](
            name=name,
        ...
        if created:
            ...
            sg = __salt__["boto3_secgroup.get_config"](
                name=name,
                group_id=None,
                region=region,
                key=key,
...
def _get_rule_changes(rules, _rules):
    ...
    for rule in rules:
        ...
        if ip_protocol not in supported_protocols and (
            ...
            raise SaltInvocationError(
                f"Invalid ip_protocol {ip_protocol} specified in security group rule."
            )
        ...
        if cidr_ip and (group_id or group_name):
            raise SaltInvocationError(
                "cidr_ip and source groups can not both be specified in security group rules."
            )
...
def _rules_present(
        ...
        for rule in rules:
            ...
            if _source_group_name:
                ...
                if _source_group_name_vpc:
                    ...
                    _group_vpc_id = None
                _group_id = __salt__["boto3_secgroup.get_group_id"](
    ...
    if to_create or to_delete:
        if __opts__["test"]:
            ...
            To be created: {}
            To be deleted: {}""".format(name, pprint.pformat(to_create), pprint.pformat(to_delete))
            ret["comment"] = msg
            ret["result"] = None
            return ret
        if to_delete:
            ...
            for rule in to_delete:
                _deleted = __salt__["boto3_secgroup.revoke"](
                    name,
                    vpc_id=vpc_id,
                    vpc_name=vpc_name,
                    region=region,
            ...
            if deleted:
                ...
                ret["comment"] = f"Failed to remove rules on {name} security group."
                ret["result"] = False
        if to_create:
            created = True
            for rule in to_create:
                _created = __salt__["boto3_secgroup.authorize"](
                    name,
                    vpc_id=vpc_id,
                    vpc_name=vpc_name,
                    region=region,
        ...
        if to_delete:
            ...
            if deleted:
                ...
                ret["comment"] = " ".join(
                    [
                        ret["comment"],
                        f"Failed to remove egress rule on {name} security group.",
                    ]
                )
                ret["result"] = False
        if to_create:
            ...
            if created:
                ...
                ret["comment"] = " ".join(
                    [
                        ret["comment"],
                        f"Failed to create egress rules on {name} security group.",
                    ]
                )
                ret["result"] = False
...
def absent(name, vpc_id=None, vpc_name=None, region=None, key=None, keyid=None, profile=None):
    """
    ...
    if sg:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        deleted = __salt__["boto3_secgroup.delete"](
            name=name,
            group_id=None,
            region=region,
            key=key,
    ...
    if tags:
        ...
        if tags_to_remove:
            if __opts__["test"]:
                ...
                temp_ret = __salt__["boto3_secgroup.delete_tags"](
                    tags_to_remove,
                    name=name,
                    group_id=None,
                    vpc_name=vpc_name,
                ...
                if not temp_ret:
                    ...
                    ret["comment"] = " ".join(
                        [
                            ret["comment"],
                            f"Error attempting to delete tags {tags_to_remove}.",
                        ]
                    )
                    return ret
        ...
        if tags_to_add or tags_to_update:
            if __opts__["test"]:
                ...
                temp_ret = __salt__["boto3_secgroup.set_tags"](
                    all_tag_changes,
                    name=name,
                    group_id=None,
                    vpc_name=vpc_name,
