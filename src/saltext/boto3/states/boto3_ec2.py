...
===============================
    ...
    Renamed from ``boto_ec2`` to ``boto3_ec2`` and updated to call the
    refactored ``boto3_ec2`` execution module.
...
Create and destroy EC2 instances. Be aware that this interacts with Amazon's
...
def __virtual__():
    ...
    """
    if "boto3_ec2.get_key" in __salt__:
        return "boto3_ec2"
    return (False, "boto3_ec2 module could not be loaded")
...
def key_present(
    ...
    if not exists:
        ...
        if save_private and not upload_public:
            created = __salt__["boto3_ec2.create_key"](
            ...
            imported = __salt__["boto3_ec2.import_key"](
...
def key_absent(name, region=None, key=None, keyid=None, profile=None):
    ...
    if exists:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        deleted = __salt__["boto3_ec2.delete_key"](name, region, key, keyid, profile)
...
def eni_present(
    ...
    if not r["result"]:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        result_create = __salt__["boto3_ec2.create_network_interface"](
            name,
            subnet_id=subnet_id,
    ...
    if allocate_eip:
        if "allocationId" not in r["result"]:
            if __opts__["test"]:
                ...
                eip_alloc = __salt__["boto3_ec2.allocate_eip_address"](
                    domain=domain, region=region, key=key, keyid=keyid, profile=profile
                )
                if eip_alloc:
                    _ret = __salt__["boto3_ec2.associate_eip_address"](
                        instance_id=None,
                        instance_name=None,
                        public_ip=None,
                    ...
                    if not _ret:
                        _ret = __salt__["boto3_ec2.release_eip_address"](
                            public_ip=None,
                            allocation_id=eip_alloc["allocation_id"],
                            region=region,
                            key=key,
...
def _eni_attribute(metadata, attr, value, region, key, keyid, profile):
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    result_update = __salt__["boto3_ec2.modify_network_interface_attribute"](
...
def _eni_groups(metadata, groups, region, key, keyid, profile):
    ...
    group_ids.sort()
    _groups = __salt__["boto3_secgroup.convert_to_group_ids"](
        groups,
...
def eni_absent(name, release_eip=False, region=None, key=None, keyid=None, profile=None):
    ...
    if not r["result"]:
        ...
        if "id" in r["result"]["attachment"]:
            result_detach = __salt__["boto3_ec2.detach_network_interface"](
                name=name,
                force=True,
                region=region,
                key=key,
        ...
        result_delete = __salt__["boto3_ec2.delete_network_interface"](
            name=name, region=region, key=key, keyid=keyid, profile=profile
        )
...
def snapshot_created(
    ...
    **kwargs,
    ...
    .. code-block:: yaml
          ...
          boto3_ec2.snapshot_created:
            - name: example

    """
    ret = {"name": name, "result": True, "comment": "", "changes": {}}

    if not __salt__["boto3_ec2.create_image"](
    ...
    starttime = time()
    while True:
        images = __salt__["boto3_ec2.find_images"](ami_name=ami_name, return_objs=True, **kwargs)
...
def instance_present(
    ...
    if instance_id:
        exists = __salt__["boto3_ec2.exists"](
    ...
    if _create:
        ...
        if image_name:
            ...
            image_ids = __salt__["boto3_ec2.find_images"](**args)
            if image_ids:
                image_id = image_ids[0]
            else:
                image_id = image_name
        r = __salt__["boto3_ec2.run"](
            image_id,
        ...
        if allocate_eip:
            ...
            r = __salt__["boto3_ec2.allocate_eip_address"](
                domain=domain, region=region, key=key, keyid=keyid, profile=profile
            )
            if not r:
                ret["result"] = False
    ...
    if public_ip or allocation_id:
        ...
        for t in range(tries):
            r = __salt__["boto3_ec2.get_eip_address_info"](
                addresses=public_ip,
        ...
        if not r:
            ret["result"] = False
            ret["comment"] = f"Failed to lookup EIP {public_ip or allocation_id}."
            return ret
        ip = r[0]["public_ip"]
        if r[0].get("instance_id"):
            ...
            if __opts__["test"]:
                ...
                ret["result"] = None
                return ret
            r = __salt__["boto3_ec2.associate_eip_address"](
                instance_id=instance_id,
                public_ip=public_ip,
    ...
    if attributes:
        for k, v in attributes.items():
            curr = __salt__["boto3_ec2.get_attribute"](
                k,
                instance_id=instance_id,
                region=region,
                key=key,
            ...
            if __opts__["test"]:
                ...
                continue
            try:
                r = __salt__["boto3_ec2.set_attribute"](
                    attribute=k,
                    attribute_value=v,
                    instance_id=instance_id,
                    region=region,
    ...
    if tags and instance_id is not None:
        tags = dict(tags)
        curr_tags = dict(
            __salt__["boto3_ec2.get_all_tags"](
        ...
        if add or remove:
            if __opts__["test"]:
                ...
                if remove:
                    if not __salt__["boto3_ec2.delete_tags"](
                        ...
                        return ret
                if add:
                    if not __salt__["boto3_ec2.create_tags"](
                        resource_ids=instance_id,
                        tags=add,
                        region=region,
                        key=key,
...
def instance_absent(
    ...
    if not instance_id:
        try:
            instance_id = __salt__["boto3_ec2.get_id"](
                name=instance_name if instance_name else name,
                region=region,
                key=key,
                keyid=keyid,
    ...
    no_can_do = __salt__["boto3_ec2.get_attribute"](
    ...
    if no_can_do.get("disableApiTermination") is True:
        ret["result"] = False
        ret["comment"] = f"Termination of instance {instance_id} via the API is disabled."
        return ret
    ...
    r = __salt__["boto3_ec2.terminate"](
        instance_id=instance_id,
        name=instance_name,
        region=region,
        key=key,
    ...
    if release_eip:
        ...
        if ip:
            ...
            if instance.get("VpcId"):
                r = __salt__["boto3_ec2.get_eip_address_info"](addresses=ip, **base_args)
            ...
            if assoc_id:
                ...
                if not __salt__["boto3_ec2.disassociate_eip_address"](
                    association_id=assoc_id, **base_args
                ):
                    log.warning("Failed to disassociate EIP %s.", ip)

            if __salt__["boto3_ec2.release_eip_address"](
...
def volume_absent(
    ...
    vols = __salt__["boto3_ec2.get_all_volumes"](filters=filters, **args)
    if len(vols) < 1:
        ret["comment"] = "Volume matching criteria not found, assuming already absent"
        return ret
    if len(vols) > 1:
        msg = f"More than one volume matched criteria, can't continue in state {name}"
        log.error(msg)
        ret["comment"] = msg
        ret["result"] = False
        return ret
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    if __salt__["boto3_ec2.delete_volume"](volume_id=vol, force=True, **args):
...
def volumes_tagged(
    ...
    if __opts__["test"]:
        args["dry_run"] = True
        r = __salt__["boto3_ec2.set_volumes_tags"](**args)
        if r["success"]:
            if r.get("changes"):
...
def volume_present(
    ...
    if not salt.utils.data.exactly_one((volume_name, volume_id)):
        raise SaltInvocationError("Exactly one of 'volume_name', 'volume_id',  must be provided.")
    ...
    if volume_name:
        ...
        if len(vols) < 1:
            if __opts__["test"]:
                ...
                ret["result"] = None
                return ret
            _rt = __salt__["boto3_ec2.create_volume"](
                zone_name=instance["Placement"]["AvailabilityZone"],
                size=size,
            ...
            if "result" in _rt:
                ...
                raise SaltInvocationError(f"Error creating volume with name {volume_name}.")
            _rt = __salt__["boto3_ec2.set_volumes_tags"](
                tag_maps=[
                    {
            ...
            volume_id = vols[0]
    vols = __salt__["boto3_ec2.get_all_volumes"](volume_ids=[volume_id], return_objs=True, **args)
    if len(vols) < 1:
        raise SaltInvocationError(f"Volume {volume_id} do not exist")
    vol = vols[0]
    instance_zone = instance["Placement"]["AvailabilityZone"]
    vol_zone = vol["AvailabilityZone"]
    if vol_zone != instance_zone:
        raise SaltInvocationError(
            "Volume {} in {} cannot attach to instance {} in {}.".format(
                volume_id, vol_zone, instance_id, instance_zone
            )
        )
    attachments = vol.get("Attachments") or []
    attach_data = attachments[0] if attachments else None
    if attach_data is not None and attach_data.get("InstanceId") is not None:
        if instance_id == attach_data["InstanceId"] and device == attach_data["Device"]:
            ...
            if __opts__["test"]:
                ...
                ret["result"] = None
                return ret
            if __salt__["boto3_ec2.detach_volume"](
                volume_id=volume_id, wait_for_detachement=True, **args
            ):
                ret["comment"] = "Volume {} is detached from {}({}).".format(
                    volume_id, attach_data["InstanceId"], attach_data["Device"]
                )
                old_dict["instance_id"] = attach_data["InstanceId"]
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    if __salt__["boto3_ec2.attach_volume"](
        volume_id=volume_id, instance_id=instance_id, device=device, **args
    ):
        ret["comment"] = " ".join(
            [
                ret["comment"],
                f"Volume {volume_id} is attached on {instance_id}({device}).",
            ]
        )
...
def private_ips_present(
    ...
    eni = __salt__["boto3_ec2.get_network_interface"](**get_eni_args)
    ...
    if ips_to_add:
        if not __opts__["test"]:
            ...
            __salt__["boto3_ec2.assign_private_ip_addresses"](**assign_ips_args)
...
def private_ips_absent(
    ...
    if ips_to_remove:
        if not __opts__["test"]:
            ...
            __salt__["boto3_ec2.unassign_private_ip_addresses"](**assign_ips_args)
