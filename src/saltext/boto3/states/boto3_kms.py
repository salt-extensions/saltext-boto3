"""
Manage KMS keys, key policies and grants using boto3.
=====================================================

    Renamed from ``boto_kms`` to ``boto3_kms`` and updated to call the
    refactored ``boto3_kms`` execution module.

Create and destroy KMS keys. Be aware that this interacts with Amazon's
...
def __virtual__():
    """
    Only load if the boto3_kms execution module is available.
    """
    if "boto3_kms.describe_key" in __salt__:
        return "boto3_kms"
    return (
        False,
        "The boto3_kms state module could not be loaded: boto3_kms execution module is unavailable.",
    )
...
def _key_present(
    ...
    alias = f"alias/{name}"
    r = __salt__["boto3_kms.key_exists"](alias, region, key, keyid, profile)
    if "error" in r:
        ret["result"] = False
        ret["comment"] = "Error when attempting to find key: {}.".format(r["error"]["message"])
        return ret
    if not r["result"]:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        rc = __salt__["boto3_kms.create_key"](
        ...
        rn = __salt__["boto3_kms.create_alias"](alias, kms_key_id, region, key, keyid, profile)
        if "error" in rn:
        ...
        rd = __salt__["boto3_kms.describe_key"](alias, region, key, keyid, profile)
        if "error" in rd:
            ret["result"] = False
...
def _key_enabled(key_metadata, enabled, region, key, keyid, profile):
    ...
    if __opts__["test"]:
        ...
        return ret
    if enabled:
        re = __salt__["boto3_kms.enable_key"](kms_key_id, region, key, keyid, profile)
        event = "Enabled"
    else:
        re = __salt__["boto3_kms.disable_key"](kms_key_id, region, key, keyid, profile)
        event = "Disabled"
    if "error" in re:
        ret["result"] = False
        ret["comment"] = "Failed to update key enabled status: {}.".format(re["error"]["message"])
    else:
...
def _key_description(key_metadata, description, region, key, keyid, profile):
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    rdu = __salt__["boto3_kms.update_key_description"](
        key_metadata["KeyId"], description, region, key, keyid, profile
    )
    if "error" in rdu:
        ret["result"] = False
        ret["comment"] = "Failed to update key description: {}.".format(rdu["error"]["message"])
    else:
...
def _key_rotation(key_metadata, key_rotation, region, key, keyid, profile):
    ret = {"result": True, "comment": "", "changes": {}}
    kms_key_id = key_metadata["KeyId"]
    rke = __salt__["boto3_kms.get_key_rotation_status"](kms_key_id, region, key, keyid, profile)
    ...
    if not key_metadata["Enabled"]:
        ...
        return ret
    if key_rotation:
        rk = __salt__["boto3_kms.enable_key_rotation"](kms_key_id, region, key, keyid, profile)
    else:
        rk = __salt__["boto3_kms.disable_key_rotation"](kms_key_id, region, key, keyid, profile)
    if "error" in rk:
        ...
        if "is disabled" in rk["error"]["message"]:
            ...
            return ret
        ret["result"] = False
        ret["comment"] = "Failed to set key rotation: {}.".format(rk["error"]["message"])
    else:
        ret["changes"] = {
...
def _key_policy(key_metadata, policy, region, key, keyid, profile):
    ret = {"result": True, "comment": "", "changes": {}}
    kms_key_id = key_metadata["KeyId"]
    rkp = __salt__["boto3_kms.get_key_policy"](kms_key_id, "default", region, key, keyid, profile)
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    rpkp = __salt__["boto3_kms.put_key_policy"](
