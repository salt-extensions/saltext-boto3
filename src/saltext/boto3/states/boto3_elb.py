...
================================================
    ...
    Renamed from ``boto_elb`` to ``boto3_elb`` and updated to call the
    refactored ``boto3_elb`` execution module.
...
def __virtual__():
    ...
    """
    if "boto3_elb.exists" in __salt__:
        return "boto3_elb"
    return (
        False,
        "The boto3_elb state module could not be loaded: boto3_elb execution module is unavailable.",
    )
...
def present(
    ...
    if ret["result"] is False:
        return ret
    exists = __salt__["boto3_elb.exists"](name, region, key, keyid, profile)
    ...
    if cnames:
        lb = __salt__["boto3_elb.get_elb_config"](name, region, key, keyid, profile)
        if lb:
            for cname in cnames:
                _ret = None
                ...
                _ret = __states__["boto3_route53.present"](**cname)
                ret.update(
                    {
                        "changes": salt.utils.dictupdate.update(ret["changes"], _ret["changes"]),
                        "comment": "  ".join([ret["comment"], _ret["comment"]]),
    ...
    if instance_names:
        ...
        for n in instance_names:
            instance_ids += __salt__["boto3_ec2.find_instances"](
                name=n,
                region=region,
                key=key,
                keyid=keyid,
...
def register_instances(name, instances, region=None, key=None, keyid=None, profile=None):
    ...
    """
    ret = {"name": name, "result": True, "comment": "", "changes": {}}
    lb = __salt__["boto3_elb.exists"](name, region, key, keyid, profile)
    if not lb:
    ...
    health = __salt__["boto3_elb.get_instance_health"](name, region, key, keyid, profile)
    nodes = [
        value["instance_id"]
        for value in health
    ...
    state = __salt__["boto3_elb.register_instances"](name, instances, region, key, keyid, profile)
    if state:
...
def _elb_present(
    ...
    for listener in listeners:
        ...
        if "instance_port" not in listener:
            raise SaltInvocationError("instance_port is a required value for listeners.")
    ...
    if subnets:
        vpc_id = __salt__["boto3_vpc.get_subnet_association"](subnets, region, key, keyid, profile)
        vpc_id = vpc_id.get("vpc_id")
        if not vpc_id:
            ret["comment"] = f"Subnets {subnets} do not map to a valid vpc id."
            ret["result"] = False
            return ret
        _security_groups = __salt__["boto3_secgroup.convert_to_group_ids"](
            security_groups,
            vpc_id=vpc_id,
            region=region,
            key=key,
        ...
        if not _security_groups:
            ret["comment"] = "Security groups {} do not map to valid security group ids.".format(
                security_groups
            )
            ret["result"] = False
            return ret
    ...
    if not exists:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        created = __salt__["boto3_elb.create"](
            name=name,
...
def _listeners_present(name, listeners, region, key, keyid, profile):
    ...
    for l in listeners:
        l_key = __salt__["boto3_elb.listener_dict_to_tuple"](l)
            ...
            for listener in to_create:
                msg.append(
                    "Listener {} added.".format(
                        __salt__["boto3_elb.listener_dict_to_tuple"](listener)
                    )
                )
    ...
    if to_delete:
        ...
        deleted = __salt__["boto3_elb.delete_listeners"](name, ports, region, key, keyid, profile)
        if deleted:
    ...
    if to_create:
        created = __salt__["boto3_elb.create_listeners"](
...
def _security_groups_present(name, security_groups, region, key, keyid, profile):
    ...
    if change_needed:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        changed = __salt__["boto3_elb.apply_security_groups"](
...
def _attributes_present(name, attributes, region, key, keyid, profile):
    ret = {"result": True, "comment": "", "changes": {}}
    _attributes = __salt__["boto3_elb.get_attributes"](name, region, key, keyid, profile)
    if not _attributes:
        ret["result"] = False
    ...
    if "connection_draining" in attributes:
        ...
        if cd["enabled"] != _cd["enabled"] or cd.get("timeout", 300) != _cd.get("timeout"):
    ...
    if "access_log" in attributes:
        ...
        if "s3_bucket_prefix" in attributes["access_log"]:
            ...
            if sbp.startswith("/") or sbp.endswith("/"):
                raise SaltInvocationError("s3_bucket_prefix can not start or end with /.")
    if attrs_to_set:
        if __opts__["test"]:
            ret["comment"] = f"ELB {name} set to have attributes set."
            ret["result"] = None
            return ret
        was_set = __salt__["boto3_elb.set_attributes"](
...
def _health_check_present(name, health_check, region, key, keyid, profile):
    ...
    if not health_check:
        health_check = {}
    _health_check = __salt__["boto3_elb.get_health_check"](name, region, key, keyid, profile)
    ...
    if need_to_set:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        was_set = __salt__["boto3_elb.set_health_check"](
            name, health_check, region, key, keyid, profile
        )
        if was_set:
            ret["changes"]["old"] = {"health_check": _health_check}
            _health_check = __salt__["boto3_elb.get_health_check"](
...
def _zones_present(name, availability_zones, region, key, keyid, profile):
    ...
    if to_enable or to_disable:
        if __opts__["test"]:
            ...
            return ret
        if to_enable:
            enabled = __salt__["boto3_elb.enable_availability_zones"](
            ...
            if enabled:
                ...
                ret["result"] = False
        if to_disable:
            disabled = __salt__["boto3_elb.disable_availability_zones"](
...
def _subnets_present(name, subnets, region, key, keyid, profile):
    ...
    if to_enable or to_disable:
        if __opts__["test"]:
            ...
            return ret
        if to_enable:
            attached = __salt__["boto3_elb.attach_subnets"](
                name, to_enable, region, key, keyid, profile
            )
            if attached:
                ...
                ret["result"] = False
        if to_disable:
            detached = __salt__["boto3_elb.detach_subnets"](
                name, to_disable, region, key, keyid, profile
            )
            if detached:
                ret["comment"] = " ".join([ret["comment"], f"Detached subnets on {name} ELB."])
            else:
                ret["comment"] = " ".join(
                    [
                        ret["comment"],
...
def _policies_present(
    ...
    for p in policies:
        ...
        if p["policy_name"] in policy_names:
            raise SaltInvocationError(
                "Policy names must be unique: policy {} is declared twice.".format(p["policy_name"])
            )
    ...
    if to_create:
        for policy_name in to_create:
            created = __salt__["boto3_elb.create_policy"](
                name=name,
    ...
    for port in listeners_to_update:
        policy_set = __salt__["boto3_elb.set_listener_policy"](
            name=name,
            port=port,
    ...
    for port in backends_to_update:
        policy_set = __salt__["boto3_elb.set_backend_policy"](
            name=name,
            port=port,
    ...
    if to_delete:
        for policy_name in to_delete:
            deleted = __salt__["boto3_elb.delete_policy"](
                name=name,
                policy_name=policy_name,
                region=region,
                key=key,
...
def absent(name, region=None, key=None, keyid=None, profile=None):
    ...
    if exists:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        deleted = __salt__["boto3_elb.delete"](name, region, key, keyid, profile)
        if deleted:
    ...
    if tags:
        ...
        if tags_to_remove:
            if __opts__["test"]:
                ...
                _ret = __salt__["boto3_elb.delete_tags"](
        ...
        if tags_to_add or tags_to_update:
            if __opts__["test"]:
                ...
                _ret = __salt__["boto3_elb.set_tags"](
