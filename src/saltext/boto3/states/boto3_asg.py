...
====================================
    ...
    Renamed from ``boto_asg`` to ``boto3_asg`` and updated to call the
    refactored ``boto3_asg`` execution module.
...
def __virtual__():
    ...
    """
    if "boto3_asg.exists" in __salt__:
        return "boto3_asg"
    return (False, "The boto3_asg state module requires the boto3_asg execution module.")
...
def present(
    ...
    alarms:
        ...
        See the :mod:`salt.states.boto3_cloudwatch_alarm` state for information
    ...
    if vpc_zone_identifier:
        vpc_id = __salt__["boto3_vpc.get_subnet_association"](
    ...
    if launch_config:
        ...
        launch_config_name = (
            launch_config_name
            + "-"
            + hashlib.md5(launch_config_bytes, usedforsecurity=False).hexdigest()
        )
        args = {
        ...
        for index, item in enumerate(launch_config):
            if "image_name" in item:
                ...
                image_ids = __salt__["boto3_ec2.find_images"](**iargs)
        ...
        if vpc_id:
            ...
            if sg_index is not None:
                ...
                _group_ids = __salt__["boto3_secgroup.convert_to_group_ids"](
    ...
    if asg is None:
        ...
        created = __salt__["boto3_asg.create"](
            name,
            launch_config_name,
            availability_zones,
            min_size,
        ...
        for asg_property, value in config.items():
            ...
            if value is None:
                continue
            value = boto3mod.ordered(value)
            if asg_property in asg:
                _value = boto3mod.ordered(asg[asg_property])
                if not value == _value:
...
def _alarms_present(
    ...
    # set alarms, using boto3_cloudwatch_alarm.present
...
def absent(name, force=False, region=None, key=None, keyid=None, profile=None, remove_lc=False):
    ...
    if asg is None:
        ...
        if __opts__["test"]:
            ...
            return ret
        deleted = __salt__["boto3_asg.delete"](name, force, region, key, keyid, profile)
        if deleted:
            if remove_lc:
                lc_deleted = __salt__["boto3_asg.delete_launch_configuration"](
