...
===================================
    ...
    Renamed from ``boto_kinesis`` to ``boto3_kinesis`` and updated to call the
    refactored ``boto3_kinesis`` execution module.
...
def __virtual__():
    ...
    """
    if "boto3_kinesis.exists" in __salt__:
        return __virtualname__
    return (
        False,
        "The boto3_kinesis state module could not be loaded: boto3_kinesis execution module is unavailable.",
    )
...
def present(
    ...
    exists = __salt__["boto3_kinesis.exists"](name, region, key, keyid, profile)
    if exists["result"] is False:
        if __opts__["test"]:
            ...
            is_created = __salt__["boto3_kinesis.create_stream"](
    ...
    stream_response = __salt__["boto3_kinesis.get_stream_when_active"](
        ...
        if not retention_matches:
            if __opts__["test"]:
                ...
                if old_retention_hours > retention_hours:
                    retention_updated = __salt__["boto3_kinesis.decrease_stream_retention_period"](
                        name, retention_hours, region, key, keyid, profile
                    )
                else:
                    retention_updated = __salt__["boto3_kinesis.increase_stream_retention_period"](
                        name, retention_hours, region, key, keyid, profile
                    )
                ...
                comments.append(f"Kinesis stream {name}: retention hours was successfully updated")
    ...
    if enhanced_monitoring is not None:
        ...
        if len(enable_metrics) != 0:
            if __opts__["test"]:
                ...
                metrics_enabled = __salt__["boto3_kinesis.enable_enhanced_monitoring"](
        ...
        if len(disable_metrics) != 0:
            if __opts__["test"]:
                ...
                metrics_disabled = __salt__["boto3_kinesis.disable_enhanced_monitoring"](
    ...
    _min_hash_key, _max_hash_key, full_stream_details = __salt__[
        "boto3_kinesis.get_info_for_reshard"
    ](stream_details)
    ...
    if num_shards is not None and do_reshard:
        ...
        if not num_shards_matches:
            if __opts__["test"]:
                ...
                while continue_reshard:
                    reshard_response = __salt__["boto3_kinesis.reshard"](
        ...
        comments.append(
            f"Kinesis stream {name}: did not reshard, remains at {old_num_shards} shards"
        )
...
def absent(name, region=None, key=None, keyid=None, profile=None):
    ...
    is_deleted = __salt__["boto3_kinesis.delete_stream"](name, region, key, keyid, profile)
