"""
Manage Lambda Functions using boto3.
====================================

    Renamed from ``boto_lambda`` to ``boto3_lambda`` and updated to call the
    refactored ``boto3_lambda`` execution module.
...
import saltext.boto3.utils.boto3mod
...
def __virtual__():
    ...
    """
    if "boto3_lambda.function_exists" in __salt__:
        return "boto3_lambda"
    return (False, "boto3_lambda module could not be loaded")
...
def function_present(
    ...
    if Permissions is not None:
        ...
        for sid, permission in Permissions.items():
            ...
            if bool(keyset):
                raise SaltInvocationError("Invalid permission value {}".format(", ".join(keyset)))

    r = __salt__["boto3_lambda.function_exists"](
    ...
    if not r.get("exists"):
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        r = __salt__["boto3_lambda.create_function"](
        ...
        if Permissions:
            for sid, permission in Permissions.items():
                r = __salt__["boto3_lambda.add_permission"](
                    FunctionName=FunctionName,
                    StatementId=sid,
                    region=region,
                    key=key,
        ...
        _describe = __salt__["boto3_lambda.describe_function"](
            FunctionName, region=region, key=key, keyid=keyid, profile=profile
        )
        _describe["function"]["Permissions"] = __salt__["boto3_lambda.get_permissions"](
            FunctionName, region=region, key=key, keyid=keyid, profile=profile
        )["permissions"]
    ...
    ret["comment"] = os.linesep.join([ret["comment"], f"Function {FunctionName} is present."])
    ret["changes"] = {}
    ...
    ret["changes"] = dictupdate.update(ret["changes"], _ret["changes"])
    ret["comment"] = " ".join([ret["comment"], _ret["comment"]])
    _ret = _function_permissions_present(FunctionName, Permissions, region, key, keyid, profile)
    if not _ret.get("result"):
        ret["result"] = _ret.get("result", False)
        ret["comment"] = _ret["comment"]
        ret["changes"] = {}
...
def _function_config_present(
    ...
    func = __salt__["boto3_lambda.describe_function"](
        FunctionName, region=region, key=key, keyid=keyid, profile=profile
    )["function"]
    need_update = False
    options = {
    ...
    if saltext.boto3.utils.boto3mod.ordered(oldval) != saltext.boto3.utils.boto3mod.ordered(
        fixed_VpcConfig
    ):
        need_update = True
    ...
    if Environment is not None:
        if func.get("Environment") != Environment:
            ...
            ret["changes"].setdefault("old", {})["Environment"] = func.get("Environment")
    ...
    if need_update:
        ...
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        _r = __salt__["boto3_lambda.update_function_config"](
            FunctionName=FunctionName,
            Role=Role,
            Handler=Handler,
            Description=Description,
        ...
        if not _r.get("updated"):
            ret["result"] = False
            ret["comment"] = "Failed to update function: {}.".format(_r["error"]["message"])
            ret["changes"] = {}
    return ret
...
def _function_code_present(
    ...
    if update:
        ...
        func = __salt__["boto3_lambda.update_function_code"](
            FunctionName,
            ZipFile,
            S3Bucket,
            S3Key,
        ...
        if not func.get("updated"):
            ret["result"] = False
            ret["comment"] = "Failed to update function: {}.".format(func["error"]["message"])
            ret["changes"] = {}
            return ret
        func = func["function"]
        if (
...
def _function_permissions_present(FunctionName, Permissions, region, key, keyid, profile):
    ret = {"result": True, "comment": "", "changes": {}}
    curr_permissions = __salt__["boto3_lambda.get_permissions"](
        FunctionName, region=region, key=key, keyid=keyid, profile=profile
    ).get("permissions")
    ...
    if bool(diffs):
        ...
        for sid, diff in diffs.items():
            ...
            if diff.get("old", "") != "":
                ...
                _r = __salt__["boto3_lambda.remove_permission"](
                    FunctionName=FunctionName,
                    StatementId=sid,
                    region=region,
                    key=key,
                ...
                ret["changes"].setdefault("new", {}).setdefault("Permissions", {})[sid] = {}
                ret["changes"].setdefault("old", {}).setdefault("Permissions", {})[sid] = diff[
                    "old"
                ]
            if diff.get("new", "") != "":
                # New permission information needs to be added
                _r = __salt__["boto3_lambda.add_permission"](
                    FunctionName=FunctionName,
                    StatementId=sid,
                    region=region,
                    key=key,
                ...
                ret["changes"].setdefault("new", {}).setdefault("Permissions", {})[sid] = diff[
                    "new"
                ]
...
def function_absent(
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    r = __salt__["boto3_lambda.delete_function"](
        FunctionName, region=region, key=key, keyid=keyid, profile=profile
    )
    if not r["deleted"]:
        ret["result"] = False
...
def alias_present(
    ...
    r = __salt__["boto3_lambda.alias_exists"](
        FunctionName=FunctionName,
        Name=Name,
        region=region,
        key=key,
    ...
    if not r.get("exists"):
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        r = __salt__["boto3_lambda.create_alias"](
            FunctionName,
            Name,
            FunctionVersion,
            Description,
        ...
        if not r.get("created"):
            ...
            ret["comment"] = "Failed to create alias: {}.".format(r["error"]["message"])
            return ret
        _describe = __salt__["boto3_lambda.describe_alias"](
    ...
    if need_update:
        ...
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        _r = __salt__["boto3_lambda.update_alias"](
            FunctionName=FunctionName,
            Name=Name,
        ...
        if not _r.get("updated"):
            ret["result"] = False
            ret["comment"] = "Failed to update alias: {}.".format(_r["error"]["message"])
            ret["changes"] = {}
    return ret
...
def alias_absent(
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    r = __salt__["boto3_lambda.delete_alias"](
        FunctionName, Name, region=region, key=key, keyid=keyid, profile=profile
    )
    if not r["deleted"]:
        ret["result"] = False
...
def event_source_mapping_present(
    ...
    r = __salt__["boto3_lambda.event_source_mapping_exists"](
    ...
    if "error" in r:
        ret["result"] = False
        ret["comment"] = "Failed to create event source mapping: {}.".format(r["error"]["message"])
        return ret

    if not r.get("exists"):
        if __opts__["test"]:
            ret["comment"] = f"Event source mapping {FunctionName} is set to be created."
            ret["result"] = None
            return ret
        r = __salt__["boto3_lambda.create_event_source_mapping"](
            EventSourceArn=EventSourceArn,
            FunctionName=FunctionName,
        ...
        if not r.get("created"):
            ...
            return ret
        _describe = __salt__["boto3_lambda.describe_event_source_mapping"](
            EventSourceArn=EventSourceArn,
            FunctionName=FunctionName,
            region=region,
            key=key,
    ...
    if need_update:
        ...
        if __opts__["test"]:
            ret["comment"] = "Event source mapping {} set to be modified.".format(_describe["UUID"])
            ret["result"] = None
            return ret
        _r = __salt__["boto3_lambda.update_event_source_mapping"](
            UUID=_describe["UUID"],
            FunctionName=FunctionName,
            Enabled=Enabled,
            BatchSize=BatchSize,
        ...
        if not _r.get("updated"):
            ret["result"] = False
            ret["comment"] = "Failed to update mapping: {}.".format(_r["error"]["message"])
            ret["changes"] = {}
    return ret
...
def event_source_mapping_absent(
    ...
    desc = __salt__["boto3_lambda.describe_event_source_mapping"](
        EventSourceArn=EventSourceArn,
        FunctionName=FunctionName,
        region=region,
        key=key,
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    r = __salt__["boto3_lambda.delete_event_source_mapping"](
        EventSourceArn=EventSourceArn,
        FunctionName=FunctionName,
        region=region,
        key=key,
    ...
    if not r["deleted"]:
        ret["result"] = False
        ret["comment"] = "Failed to delete event source mapping: {}.".format(r["error"]["message"])
        return ret
