"""
Manage S3 Buckets using boto3.
==============================

    Renamed from ``boto_s3_bucket`` to ``boto3_s3_bucket`` and updated to call the
    refactored ``boto3_s3_bucket`` execution module.
...
def __virtual__():
    ...
    """
    if "boto3_s3_bucket.exists" in __salt__:
        return "boto3_s3_bucket"
    return (False, "boto3_s3_bucket module could not be loaded")
...
def _get_canonical_id(region, key, keyid, profile):
    ret = __salt__["boto3_s3_bucket.list"](
...
def _acl_to_grant(ACL, owner_canonical_id):
    ...
    if not ret["Grants"]:
        ret["Grants"] = [{"Grantee": owner_canonical_grant, "Permission": "FULL_CONTROL"}]
    return ret
...
def _compare_json(current, desired, *_args, **_kwargs):
    return boto3mod.json_objs_equal(current, desired)
...
def _compare_acl(current, desired, region, key, keyid, profile):
    ...
    return boto3mod.json_objs_equal(current, _acl_to_grant(desired, ocid))


def _compare_policy(current, desired, *_args, **_kwargs):
...
def present(
    ...
    if Policy:
        ...
        Policy = boto3mod.ordered(Policy)

    r = __salt__["boto3_s3_bucket.exists"](
    ...
    if not r.get("exists"):
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        r = __salt__["boto3_s3_bucket.create"](
            Bucket=Bucket,
        ...
        for setter, testval, funcargs in (
            ...
            if testval is not None:
                r = __salt__[f"boto3_s3_bucket.{setter}"](
                    Bucket=Bucket,
                    region=region,
                    key=key,
                    keyid=keyid,
        ...
        _describe = __salt__["boto3_s3_bucket.describe"](
    ...
    if "error" in _describe:
        ret["result"] = False
        ret["comment"] = "Failed to update bucket: {}.".format(_describe["error"]["message"])
        ret["changes"] = {}
        return ret
        ...
        if varname == "Policy":
            if current is not None:
                ...
                if isinstance(temp, str):
                    current = boto3mod.ordered({"Policy": salt.utils.json.loads(temp)})
        if not comparator(current, desired, region, key, keyid, profile):
            ...
            if not __opts__["test"]:
                if deleter and desired is None:
                    ...
                    r = __salt__[f"boto3_s3_bucket.{deleter}"](
                        Bucket=Bucket,
                        region=region,
                        key=key,
                        keyid=keyid,
...
def absent(
    name, Bucket, Force=False, region=None, key=None, keyid=None, profile=None
):  # pylint: disable=unused-argument
    """
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    r = __salt__["boto3_s3_bucket.delete"](
