"""
Manage Apigateway Rest APIs using boto3.
========================================

    Renamed from ``boto_apigateway`` to ``boto3_apigateway`` and updated to call the
    refactored ``boto3_apigateway`` execution module.
...
This module accepts explicit Apigateway credentials but can also utilize
...
def __virtual__():
    """
    Only load if the boto3_apigateway execution module is available.
    """
    if "boto3_apigateway.describe_apis" in __salt__:
        return "boto3_apigateway"
    return (
        False,
        "The boto3_apigateway state module requires the boto3_apigateway execution module.",
    )
...
def present(
    ...
    lambda_region
        ...
        2. if lambda_region is None, use the region as if a boto3_lambda
...
def absent(
    ...
    """
    Ensure the stage_name associated with the given api_name deployed by boto3_apigateway's
...
def _gen_md5_filehash(fname, *args):
    ...
    """
    _hash = hashlib.md5(usedforsecurity=False)
...
class _Swagger:
    ...
    AWS_API_DESCRIPTION = _dict_to_json_pretty(
        {
            "provisioned_by": "Salt boto3_apigateway.present State",
    ...
    class SwaggerParameter:
        ...
        def location(self):
            ...
            if _location in _Swagger.SwaggerParameter.LOCATIONS:
                return _location
            raise ValueError(f"Unsupported parameter location: {_location} in Parameter Object")
        ...
        def name(self):
            ...
            if _name:
                ...
                return None
            raise ValueError(f"Parameter must have a name: {_dict_to_json_pretty(self._paramdict)}")
            ...
            for opname, opobj in ops.items():
                ...
                for rescode, resobj in opobj.get("responses").items():
                    ...
                    if model.get("type") != "object":
                        raise ValueError(f"model schema {modelname} must be type object")
                    if "properties" not in model:
                        raise ValueError(f"model schema {modelname} must have properties fields")
    ...
    def _validate_lambda_funcname_format(self):
        ...
        try:
            if self._lambda_funcname_format:
                known_kwargs = {"stage": "", "api": "", "resource": "", "method": ""}
    ...
    def paths(self):
        ...
        for path in paths:
            if not path.startswith("/"):
                raise ValueError(f"Path object {path} should start with /. Please fix it")
        return paths.items()
    ...
    # methods to interact with boto3_apigateway execution modules
    def _one_or_more_stages_remain(self, deploymentId):
        ...
        """
        stages = __salt__["boto3_apigateway.describe_api_stages"](
    ...
    def no_more_deployments_remain(self):
        ...
        deployments = __salt__["boto3_apigateway.describe_api_deployments"](
    ...
    def _get_current_deployment_id(self):
        ...
        """
        deploymentId = ""
        stage = __salt__["boto3_apigateway.describe_api_stage"](
            restApiId=self.restApiId,
    ...
    def _get_current_deployment_label(self):
        ...
        deployment = __salt__["boto3_apigateway.describe_api_deployment"](
            restApiId=self.restApiId, deploymentId=deploymentId, **self._common_aws_args
        ).get("deployment")
        if deployment:
    ...
    def overwrite_stage_variables(self, ret, stage_variables):
        ...
        """
        res = __salt__["boto3_apigateway.overwrite_api_stage_variables"](
            restApiId=self.restApiId,
            stageName=self._stage_name,
    ...
    def _set_current_deployment(self, stage_desc_json, stage_variables):
        ...
        if not stage:
            stage = __salt__["boto3_apigateway.create_api_stage"](
                restApiId=self.restApiId,
                stageName=self._stage_name,
            ...
            overwrite = __salt__["boto3_apigateway.overwrite_api_stage_variables"](
                restApiId=self.restApiId,
                stageName=self._stage_name,
                variables=stage_variables,
                **self._common_aws_args,
        ...
        return __salt__["boto3_apigateway.activate_api_deployment"](
            restApiId=self.restApiId,
            stageName=self._stage_name,
            deploymentId=self._deploymentId,
            **self._common_aws_args,
    ...
    def _resolve_api_id(self):
        ...
        """
        apis = __salt__["boto3_apigateway.describe_apis"](
    ...
    def delete_stage(self, ret):
        ...
        deploymentId = self._get_current_deployment_id()
        if deploymentId:
            result = __salt__["boto3_apigateway.delete_api_stage"](
                restApiId=self.restApiId,
                stageName=self._stage_name,
                **self._common_aws_args,
            )
            if not result.get("deleted"):
                ret["abort"] = True
                ret["result"] = False
                ret["comment"] = "delete_stage delete_api_stage, {}".format(result.get("error"))
            else:
                # check if it is safe to delete the deployment as well.
                if not self._one_or_more_stages_remain(deploymentId):
                    result = __salt__["boto3_apigateway.delete_api_deployment"](
                        restApiId=self.restApiId,
                        deploymentId=deploymentId,
                        **self._common_aws_args,
                    )
                    if not result.get("deleted"):
                        ret["abort"] = True
                        ret["result"] = False
                        ret["comment"] = "delete_stage delete_api_deployment, {}".format(
                            result.get("error")
                        )
                else:
                    ret["comment"] = f"stage {self._stage_name} has been deleted.\n"
        else:
        ...
        if self.restApiId:
            ...
            if deployed_label_json == self.deployment_label_json:
                ret["comment"] = (
                    ...
                    "deployment label:\n{}".format(self._stage_name, deployed_label_json)
                )
                ret["current"] = True
                return ret
            else:
    ...
    def publish_api(self, ret, stage_variables):
        ...
        if self._deploymentId:
            ...
            res = __salt__["boto3_apigateway.create_api_deployment"](
                restApiId=self.restApiId,
                stageName=self._stage_name,
    ...
    def _cleanup_api(self):
        ...
        for a stage
        """
        resources = __salt__["boto3_apigateway.describe_api_resources"](
            restApiId=self.restApiId, **self._common_aws_args
        )
        if resources.get("resources"):
            ...
            res.reverse()
            for resource in res:
                delres = __salt__["boto3_apigateway.delete_api_resources"](
                    restApiId=self.restApiId,
        ...
        models = __salt__["boto3_apigateway.describe_api_models"](
            restApiId=self.restApiId, **self._common_aws_args
        )
        if models.get("models"):
            for model in models.get("models"):
                delres = __salt__["boto3_apigateway.delete_api_model"](
                    restApiId=self.restApiId,
    ...
    def deploy_api(self, ret):
        ...
        response = __salt__["boto3_apigateway.create_api"](
            name=self.rest_api_name,
            description=_Swagger.AWS_API_DESCRIPTION,
            **self._common_aws_args,
        )
    ...
    def delete_api(self, ret):
        ...
        exists_response = __salt__["boto3_apigateway.api_exists"](
            name=self.rest_api_name,
            description=_Swagger.AWS_API_DESCRIPTION,
            **self._common_aws_args,
        )
        if exists_response.get("exists"):
            ...
            delete_api_response = __salt__["boto3_apigateway.delete_api"](
                name=self.rest_api_name,
                description=_Swagger.AWS_API_DESCRIPTION,
                **self._common_aws_args,
            )
    ...
    def deploy_models(self, ret):
        ...
        for model, schema in self.models():
            ...
            model_exists_response = __salt__["boto3_apigateway.api_model_exists"](
                restApiId=self.restApiId, modelName=model, **self._common_aws_args
            )

            if model_exists_response.get("exists"):
                update_model_schema_response = __salt__["boto3_apigateway.update_api_model_schema"](
                    restApiId=self.restApiId,
                    modelName=model,
                ...
                create_model_response = __salt__["boto3_apigateway.create_api_model"](
                    restApiId=self.restApiId,
                    modelName=model,
                ...
                if not create_model_response.get("created"):
                    ...
                    if "error" in create_model_response:
                        ret["comment"] = "Failed to create model {}, schema {}, error: {}".format(
                            model,
                            _dict_to_json_pretty(schema),
    ...
    def _lambda_name(self, resourcePath, httpMethod):
        ...
        boto3_apigateway.api_present function
        """
    ...
    def _lambda_uri(self, lambda_name, lambda_region):
        ...
        lambda_region = boto3mod.get_region("lambda", lambda_region, profile, opts=__opts__)
        apigw_region = boto3mod.get_region("apigateway", region, profile, opts=__opts__)

        lambda_desc = __salt__["boto3_lambda.describe_function"](
    ...
    def _parse_method_response(self, method_name, method_response, httpStatus):
        ...
        for header in method_response.headers:
            ...
            method_integration_response_params[response_header] = (
                "'{}'".format(header_data.get("default")) if "default" in header_data else "'*'"
            )
    ...
    def _deploy_method(
        ...
        m = __salt__["boto3_apigateway.create_api_method"](
            restApiId=self.restApiId,
        ...
        integration = __salt__["boto3_apigateway.create_api_integration"](
            restApiId=self.restApiId,
            resourcePath=resource_path,
            httpMethod=method_name.upper(),
        ...
        if "responses" in method_data:
            for response, response_data in method_data["responses"].items():
                ...
                mr = __salt__["boto3_apigateway.create_api_method_response"](
                    restApiId=self.restApiId,
                    resourcePath=resource_path,
                    httpMethod=method_name.upper(),
                ...
                mir = __salt__["boto3_apigateway.create_api_integration_response"](
                    restApiId=self.restApiId,
                    resourcePath=resource_path,
                    httpMethod=method_name.upper(),
                    statusCode=httpStatus,
            ...
            raise ValueError(f"No responses specified for {resource_path} {method_name}")
    ...
    def deploy_resources(
        ...
        for path, pathData in self.paths:
            resource = __salt__["boto3_apigateway.create_api_resources"](
...
def usage_plan_present(
    ...
    .. code-block:: yaml
        ...
        UsagePlanPresent:
          boto3_apigateway.usage_plan_present:
    ...
    try:
        ...
        existing = __salt__["boto3_apigateway.describe_usage_plans"](name=plan_name, **common_args)
        ...
        if not existing["plans"]:
            ...
            if __opts__["test"]:
                ret["comment"] = f"a new usage plan {plan_name} would be created"
                ret["result"] = None
                return ret

            result = __salt__["boto3_apigateway.create_usage_plan"](
                name=plan_name,
            ...
            if not needs_updating:
                ret["comment"] = f"usage plan {plan_name} is already in a correct state"
                ret["result"] = True
                return ret

            if __opts__["test"]:
                ret["comment"] = f"a new usage plan {plan_name} would be updated"
                ret["result"] = None
                return ret

            result = __salt__["boto3_apigateway.update_usage_plan"](
        ...
        newstate = __salt__["boto3_apigateway.describe_usage_plans"](name=plan_name, **common_args)
        if "error" in existing:
            ret["result"] = False
...
def usage_plan_absent(name, plan_name, region=None, key=None, keyid=None, profile=None):
    ...
    .. code-block:: yaml
        ...
        usage plan absent:
          boto3_apigateway.usage_plan_absent:
            - plan_name: my_usage_plan
            - profile: my_profile
    ...
    try:
        ...
        if __opts__["test"]:
            ret["comment"] = f"Usage plan {plan_name} exists and would be deleted"
            ret["result"] = None
            return ret

        plan_id = existing["plans"][0]["id"]
        result = __salt__["boto3_apigateway.delete_usage_plan"](plan_id, **common_args)

        if "error" in result:
            ret["result"] = False
            ret["comment"] = f"Failed to delete usage plan {plan_name}, {result}"
            return ret
...
def usage_plan_association_present(
    ...
    .. code-block:: yaml
        ...
        UsagePlanAssociationPresent:
          boto3_apigateway.usage_plan_association_present:
            - plan_name: my_plan
            - api_stages:
              - apiId: 9kb0404ec0
                stage: my_stage
    ...
    try:
        ...
        result = __salt__["boto3_apigateway.attach_usage_plan_to_apis"](
            plan_id, stages_to_add, **common_args
        )
        if "error" in result:
            ret["comment"] = "Failed to associate a usage plan {} to the apis {}, {}".format(
...
def usage_plan_association_absent(
    ...
    .. code-block:: yaml
        ...
        UsagePlanAssociationAbsent:
          boto3_apigateway.usage_plan_association_absent:
            - plan_name: my_plan
            - api_stages:
              - apiId: 9kb0404ec0
                stage: my_stage
    ...
    try:
        ...
        if not plan_stages:
            ret["comment"] = f"Usage plan {plan_name} has no associated stages already"
            return ret
        ...
        result = __salt__["boto3_apigateway.detach_usage_plan_from_apis"](
            plan_id, stages_to_remove, **common_args
        )
        if "error" in result:
            ret["comment"] = "Failed to disassociate a usage plan {} from the apis {}, {}".format(
