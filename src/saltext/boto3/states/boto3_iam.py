...
===============================
    ...
    Renamed from ``boto_iam`` to ``boto3_iam`` and updated to call the
    refactored ``boto3_iam`` execution module.
...
def __virtual__():
    ...
    """
    if "boto3_iam.get_user" in __salt__:
        return True
    else:
        return (
            False,
            f"Cannot load {__virtualname__} state: boto3_iam module unavailable",
        )
...
def user_absent(
    ...
    if delete_keys:
        keys = __salt__["boto3_iam.get_all_access_keys"](
            user_name=name, region=region, key=key, keyid=keyid, profile=profile
        )
        log.debug("Keys for user %s are %s.", name, keys)
        if isinstance(keys, dict):
            keys = keys.get("AccessKeyMetadata", [])
            for k in keys:
                if __opts__["test"]:
                    ret["comment"] = " ".join(
                        [
                            ret["comment"],
                            "Key {} is set to be deleted.".format(k["AccessKeyId"]),
                        ]
                    )
                    ret["result"] = None
                else:
                    if _delete_key(ret, k["AccessKeyId"], name, region, key, keyid, profile):
                        ret["comment"] = " ".join(
                            [
                                ret["comment"],
                                "Key {} has been deleted.".format(k["AccessKeyId"]),
                            ]
                        )
                        ret["changes"][k["AccessKeyId"]] = "deleted"
    # delete the user's MFA tokens
    if delete_mfa_devices:
        devices = __salt__["boto3_iam.get_all_mfa_devices"](
            user_name=name, region=region, key=key, keyid=keyid, profile=profile
        )
        if devices:
            for d in devices:
                ...
                if __opts__["test"]:
                    ...
                    mfa_deactivated = __salt__["boto3_iam.deactivate_mfa_device"](
                        user_name=name,
                        serial=serial,
                        region=region,
                        key=key,
                    ...
                    if mfa_deactivated:
                        ret["comment"] = " ".join(
                            [
                                ret["comment"],
                                f"IAM user {name} MFA device {serial} is deactivated.",
                            ]
                        )
                if __opts__["test"]:
                    ret["comment"] = " ".join(
                        [
                            ret["comment"],
                            f"Virtual MFA device {serial} is set to be deleted.",
                        ]
                    )
                    ret["result"] = None
                else:
                    mfa_deleted = __salt__["boto3_iam.delete_virtual_mfa_device"](
                        serial=serial,
                        region=region,
                        key=key,
                        keyid=keyid,
    ...
    if delete_profile:
        if __opts__["test"]:
            ...
            profile_deleted = __salt__["boto3_iam.delete_login_profile"](
    ...
    if __opts__["test"]:
        ret["comment"] = " ".join([ret["comment"], f"IAM user {name} is set to be deleted."])
        ret["result"] = None
        return ret
    deleted = __salt__["boto3_iam.delete_user"](name, region, key, keyid, profile)
    if deleted is True:
...
def keys_present(
    ...
    for i in range(number - len(keys)):
        created = __salt__["boto3_iam.create_access_key"](name, region, key, keyid, profile)
        ...
        new_keys[str(i)] = {}
        new_keys[str(i)]["key_id"] = created["AccessKey"]["AccessKeyId"]
        new_keys[str(i)]["secret_key"] = created["AccessKey"]["SecretAccessKey"]
    try:
...
def _delete_key(ret, access_key_id, user_name, region=None, key=None, keyid=None, profile=None):
    ...
    if isinstance(keys, str):
        ...
        ret["comment"] = " ".join([ret["comment"], f"Key {access_key_id} could not be deleted."])
        return ret
    keys = keys.get("AccessKeyMetadata", [])
    for k in keys:
        log.debug("Key is: %s and is compared with: %s", k["AccessKeyId"], access_key_id)
        if str(k["AccessKeyId"]) == str(access_key_id):
            if __opts__["test"]:
                ret["comment"] = f"Access key {access_key_id} is set to be deleted."
                ret["result"] = None
                return ret
            deleted = __salt__["boto3_iam.delete_access_key"](
...
def user_present(
    ...
    exists = __salt__["boto3_iam.get_user"](name, region, key, keyid, profile)
    if not exists:
        if __opts__["test"]:
            ret["comment"] = f"IAM user {name} is set to be created."
            ret["result"] = None
            return ret
        created = __salt__["boto3_iam.create_user"](name, path, region, key, keyid, profile)
        if created:
            ret["changes"]["user"] = created
            ret["comment"] = " ".join([ret["comment"], f"User {name} has been created."])
            if password:
...
def _user_policies_present(name, policies=None, region=None, key=None, keyid=None, profile=None):
    ...
    for policy_name, policy in policies.items():
        if isinstance(policy, str):
            dict_policy = salt.utils.json.loads(policy, object_pairs_hook=odict.OrderedDict)
        else:
            dict_policy = policy
        _policy = __salt__["boto3_iam.get_user_policy"](
    ...
    _list = __salt__["boto3_iam.get_all_user_policies"](
        user_name=name, region=region, key=key, keyid=keyid, profile=profile
    )
    ...
    if policies_to_create or policies_to_delete:
        ...
        for policy_name, policy in policies_to_create.items():
            policy_set = __salt__["boto3_iam.put_user_policy"](
            ...
            if not policy_set:
                ...
                ret["result"] = False
                ret["comment"] = f"Failed to add policy {policy_name} for user {name}"
                return ret
        for policy_name in policies_to_delete:
            policy_unset = __salt__["boto3_iam.delete_user_policy"](
                name, policy_name, region, key, keyid, profile
            )
            if not policy_unset:
                _list = __salt__["boto3_iam.get_all_user_policies"](
                ...
                ret["changes"]["new"] = {"policies": _list}
                ret["result"] = False
                ret["comment"] = f"Failed to add policy {policy_name} to user {name}"
                return ret
        _list = __salt__["boto3_iam.get_all_user_policies"](
            user_name=name, region=region, key=key, keyid=keyid, profile=profile
        )
        ret["changes"]["new"] = {"policies": _list}
        ret["comment"] = "{} policies modified on user {}.".format(", ".join(_list), name)
    return ret
...
def _user_policies_attached(
    ...
    for policy in managed_policies or []:
        entities = __salt__["boto3_iam.list_entities_for_policy"](
            policy,
            entity_filter="User",
            region=region,
            key=key,
        ...
        for userdict in entities.get("policy_users", []):
            if name == userdict.get("UserName"):
                found = True
                break
        if not found:
            policies_to_attach.append(policy)
    _list = __salt__["boto3_iam.list_attached_user_policies"](
        name, region=region, key=key, keyid=keyid, profile=profile
    )
    oldpolicies = [x.get("PolicyArn") for x in _list]
    for policy_data in _list:
        if (
            policy_data.get("PolicyName") not in managed_policies
            and policy_data.get("PolicyArn") not in managed_policies
        ):
            policies_to_detach.append(policy_data.get("PolicyArn"))
    if policies_to_attach or policies_to_detach:
        ...
        for policy_name in policies_to_attach:
            policy_set = __salt__["boto3_iam.attach_user_policy"](
            ...
            if not policy_set:
                ...
                newpolicies = [x.get("PolicyArn") for x in _list]
                ret["changes"]["new"] = {"managed_policies": newpolicies}
                ret["result"] = False
                ret["comment"] = f"Failed to add policy {policy_name} to user {name}"
                return ret
        for policy_name in policies_to_detach:
            policy_unset = __salt__["boto3_iam.detach_user_policy"](
                policy_name, name, region=region, key=key, keyid=keyid, profile=profile
            )
            if not policy_unset:
                _list = __salt__["boto3_iam.list_attached_user_policies"](
                ...
                ret["changes"]["new"] = {"managed_policies": newpolicies}
                ret["result"] = False
                ret["comment"] = f"Failed to remove policy {policy_name} from user {name}"
                return ret
        _list = __salt__["boto3_iam.list_attached_user_policies"](
            name, region=region, key=key, keyid=keyid, profile=profile
        )
        ...
        ret["changes"]["new"] = {"managed_policies": newpolicies}
        ret["comment"] = "{} policies modified on user {}.".format(", ".join(newpolicies), name)
    return ret
...
def _user_policies_detached(name, region=None, key=None, keyid=None, profile=None):
    ...
    newpolicies = [x.get("PolicyArn") for x in _list]
    ret["changes"]["new"] = {"managed_policies": newpolicies}
    ret["comment"] = "{} policies detached from user {}.".format(", ".join(oldpolicies), name)
    return ret


def _user_policies_deleted(name, region=None, key=None, keyid=None, profile=None):
    ret = {"result": True, "comment": "", "changes": {}}
    oldpolicies = __salt__["boto3_iam.get_all_user_policies"](
        user_name=name, region=region, key=key, keyid=keyid, profile=profile
    )
    if not oldpolicies:
    ...
    for policy_name in oldpolicies:
        policy_deleted = __salt__["boto3_iam.delete_user_policy"](
            name, policy_name, region=region, key=key, keyid=keyid, profile=profile
        )
        if not policy_deleted:
            newpolicies = __salt__["boto3_iam.get_all_user_policies"](
                name, region=region, key=key, keyid=keyid, profile=profile
            )
            ret["changes"]["new"] = {"inline_policies": newpolicies}
            ret["result"] = False
            ret["comment"] = f"Failed to detach {policy_name} from user {name}"
            return ret
    newpolicies = __salt__["boto3_iam.get_all_user_policies"](
        name, region=region, key=key, keyid=keyid, profile=profile
    )
    ret["changes"]["new"] = {"inline_policies": newpolicies}
    ret["comment"] = "{} policies deleted from user {}.".format(", ".join(oldpolicies), name)
    return ret
...
def _case_password(ret, name, password, region=None, key=None, keyid=None, profile=None):
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    login = __salt__["boto3_iam.create_login_profile"](name, password, region, key, keyid, profile)
    log.debug("Login is : %s.", login)
    if login:
        if "Conflict" in login:
            ret["comment"] = " ".join([ret["comment"], f"Login profile for user {name} exists."])
        else:
            ret["comment"] = " ".join([ret["comment"], f"Password has been added to User {name}."])
...
def group_absent(name, region=None, key=None, keyid=None, profile=None):
    ...
    if __opts__["test"]:
        ...
        if not _ret["result"]:
            ...
            if ret["result"] is False:
                return ret
    ret["comment"] = " ".join([ret["comment"], f"IAM group {name} users are set to be removed."])
    existing_users = __salt__["boto3_iam.get_group_members"](
    ...
    if __opts__["test"]:
        ret["comment"] = " ".join([ret["comment"], f"IAM group {name} is set to be deleted."])
        ret["result"] = None
        return ret
    deleted = __salt__["boto3_iam.delete_group"](name, region, key, keyid, profile)
    if deleted is True:
...
def group_present(
    ...
    for policy in policies_from_pillars:
        ...
        _policies.update(_policy)
    _policies.update(policies)
    exists = __salt__["boto3_iam.get_group"](
        group_name=name, region=region, key=key, keyid=keyid, profile=profile
    )
    if not exists:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        created = __salt__["boto3_iam.create_group"](
            group_name=name,
            path=path,
            region=region,
            key=key,
    ...
    _ret = _group_policies_present(name, _policies, region, key, keyid, profile, delete_policies)
    ret["changes"] = dictupdate.update(ret["changes"], _ret["changes"])
    ret["comment"] = " ".join([ret["comment"], _ret["comment"]])
    if not _ret["result"]:
        ret["result"] = _ret["result"]
...
def _case_group(ret, users, group_name, existing_users, region, key, keyid, profile):
    ...
    for user in users:
        ...
        if __opts__["test"]:
            ret["comment"] = f"User {user} is set to be added to group {group_name}."
            ret["result"] = None
        else:
            __salt__["boto3_iam.add_user_to_group"](user, group_name, region, key, keyid, profile)
            ret["comment"] = " ".join(
                [
                    ret["comment"],
        ...
        if user not in users:
            if __opts__["test"]:
                ret["comment"] = " ".join(
                    [
                        ret["comment"],
                        f"User {user} is set to be removed from group {group_name}.",
                    ]
                )
                ret["result"] = None
            else:
                __salt__["boto3_iam.remove_user_from_group"](
                ...
                ret["comment"] = " ".join(
                    [
                        ret["comment"],
                        f"User {user} has been removed from group {group_name}.",
                    ]
                )
...
def _group_policies_present(
    ...
    for policy_name, policy in policies.items():
        if isinstance(policy, str):
            ...
            dict_policy = policy
        _policy = __salt__["boto3_iam.get_group_policy"](
            name, policy_name, region, key, keyid, profile
        )
        if _policy != dict_policy:
            log.debug("Policy mismatch:\n%s\n%s", _policy, dict_policy)
            policies_to_create[policy_name] = policy
    _list = __salt__["boto3_iam.get_all_group_policies"](name, region, key, keyid, profile)
    for policy_name in _list:
    ...
    if policies_to_create or policies_to_delete:
        ...
        ret["changes"]["old"] = {"policies": _list}
        for policy_name, policy in policies_to_create.items():
            policy_set = __salt__["boto3_iam.put_group_policy"](
                name, policy_name, policy, region, key, keyid, profile
            )
            if not policy_set:
                _list = __salt__["boto3_iam.get_all_group_policies"](
                    name, region, key, keyid, profile
                )
                ret["changes"]["new"] = {"policies": _list}
                ret["result"] = False
                ret["comment"] = f"Failed to add policy {policy_name} to group {name}"
                return ret
        for policy_name in policies_to_delete:
            policy_unset = __salt__["boto3_iam.delete_group_policy"](
                name, policy_name, region, key, keyid, profile
            )
            if not policy_unset:
                _list = __salt__["boto3_iam.get_all_group_policies"](
        ...
        _list = __salt__["boto3_iam.get_all_group_policies"](name, region, key, keyid, profile)
        ret["changes"]["new"] = {"policies": _list}
        ret["comment"] = "{} policies modified on group {}.".format(", ".join(_list), name)
    return ret
...
def _group_policies_attached(
    ...
    for policy in managed_policies or []:
        ...
        for groupdict in entities.get("policy_groups", []):
            if name == groupdict.get("GroupName"):
                found = True
                break
        if not found:
            policies_to_attach.append(policy)
    _list = __salt__["boto3_iam.list_attached_group_policies"](
        name, region=region, key=key, keyid=keyid, profile=profile
    )
    oldpolicies = [x.get("PolicyArn") for x in _list]
    for policy_data in _list:
        if (
            detach_policies
            and policy_data.get("PolicyName") not in managed_policies
            and policy_data.get("PolicyArn") not in managed_policies
        ):
            policies_to_detach.append(policy_data.get("PolicyArn"))
    if policies_to_attach or policies_to_detach:
        ...
        ret["changes"]["old"] = {"managed_policies": oldpolicies}
        for policy_name in policies_to_attach:
            policy_set = __salt__["boto3_iam.attach_group_policy"](
                policy_name, name, region=region, key=key, keyid=keyid, profile=profile
            )
            if not policy_set:
                _list = __salt__["boto3_iam.list_attached_group_policies"](
                ...
                return ret
        for policy_name in policies_to_detach:
            policy_unset = __salt__["boto3_iam.detach_group_policy"](
                policy_name, name, region=region, key=key, keyid=keyid, profile=profile
            )
            if not policy_unset:
                _list = __salt__["boto3_iam.list_attached_group_policies"](
        ...
        log.debug(newpolicies)
        ret["changes"]["new"] = {"managed_policies": newpolicies}
        ret["comment"] = "{} policies modified on group {}.".format(", ".join(newpolicies), name)
    return ret
...
def _group_policies_detached(name, region=None, key=None, keyid=None, profile=None):
    ...
    for policy_arn in oldpolicies:
        ...
        if not policy_unset:
            ...
            ret["changes"]["new"] = {"managed_policies": newpolicies}
            ret["result"] = False
            ret["comment"] = f"Failed to detach {policy_arn} from group {name}"
            return ret
    _list = __salt__["boto3_iam.list_attached_group_policies"](
        name, region=region, key=key, keyid=keyid, profile=profile
    )
    newpolicies = [x.get("PolicyArn") for x in _list]
    ret["changes"]["new"] = {"managed_policies": newpolicies}
    ret["comment"] = "{} policies detached from group {}.".format(", ".join(newpolicies), name)
    return ret


def _group_policies_deleted(name, region=None, key=None, keyid=None, profile=None):
    ret = {"result": True, "comment": "", "changes": {}}
    oldpolicies = __salt__["boto3_iam.get_all_group_policies"](
        group_name=name, region=region, key=key, keyid=keyid, profile=profile
    )
    if not oldpolicies:
    ...
    ret["changes"]["old"] = {"inline_policies": oldpolicies}
    for policy_name in oldpolicies:
        policy_deleted = __salt__["boto3_iam.delete_group_policy"](
            name, policy_name, region=region, key=key, keyid=keyid, profile=profile
        )
        if not policy_deleted:
            newpolicies = __salt__["boto3_iam.get_all_group_policies"](
                name, region=region, key=key, keyid=keyid, profile=profile
            )
            ret["changes"]["new"] = {"inline_policies": newpolicies}
            ret["result"] = False
            ret["comment"] = f"Failed to detach {policy_name} from group {name}"
            return ret
    newpolicies = __salt__["boto3_iam.get_all_group_policies"](
        name, region=region, key=key, keyid=keyid, profile=profile
    )
    ret["changes"]["new"] = {"inline_policies": newpolicies}
    ret["comment"] = "{} policies deleted from group {}.".format(", ".join(oldpolicies), name)
    return ret
...
def account_policy(
    ...
    info = __salt__["boto3_iam.get_account_policy"](region, key, keyid, profile)
    if not info:
    ...
    for key, value in config.items():
        if key in ("region", "key", "keyid", "profile", "name"):
            continue
        # Map snake_case to PascalCase used by boto3 IAM response.
        pascal = "".join(part.capitalize() for part in key.split("_"))
        existing = info.get(pascal)
        if value is not None and str(existing).lower() != str(value).lower():
            ret["comment"] = " ".join(
                [
                    ret["comment"],
                    f"Policy value {value} has been set to {existing}.",
                ]
            )
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    if __salt__["boto3_iam.update_account_password_policy"](
...
def server_cert_absent(name, region=None, key=None, keyid=None, profile=None):
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    deleted = __salt__["boto3_iam.delete_server_cert"](name, region, key, keyid, profile)
    if not deleted:
        ret["result"] = False
...
def server_cert_present(
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    created = __salt__["boto3_iam.upload_server_cert"](
...
def policy_present(
    ...
    if not policy:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        created = __salt__["boto3_iam.create_policy"](
            name, policy_document, path, description, region, key, keyid, profile
        )
        if created:
            ret["changes"]["policy"] = created
            ret["comment"] = " ".join([ret["comment"], f"Policy {name} has been created."])
        else:
            ret["result"] = False
        ...
        _describe = __salt__["boto3_iam.get_policy_version"](
            name, policy.get("DefaultVersionId"), region, key, keyid, profile
        ).get("policy_version", {})
        ...
        if bool(r):
            ...
            r = __salt__["boto3_iam.create_policy_version"](
                policy_name=name,
            ...
            __salt__["boto3_iam.delete_policy_version"](
                policy_name=name,
...
def policy_absent(name, region=None, key=None, keyid=None, profile=None):
    ...
    r = __salt__["boto3_iam.policy_exists"](
        name, region=region, key=key, keyid=keyid, profile=profile
    )
    if not r:
    ...
    versions = __salt__["boto3_iam.list_policy_versions"](
        name, region=region, key=key, keyid=keyid, profile=profile
    )
    if versions:
        for version in versions:
            if version.get("IsDefaultVersion", False) in ("true", True):
                continue
            r = __salt__["boto3_iam.delete_policy_version"](
                name,
            ...
            if not r:
                ...
                return ret
    r = __salt__["boto3_iam.delete_policy"](
        name, region=region, key=key, keyid=keyid, profile=profile
    )
    if not r:
        ret["result"] = False
...
def saml_provider_present(
    ...
    if "salt://" in saml_metadata_document:
        try:
            ...
            log.debug(e)
            ret["comment"] = f"SAML document file {name} not found or could not be loaded"
            ret["result"] = False
            return ret
    for provider in __salt__["boto3_iam.list_saml_providers"](
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    created = __salt__["boto3_iam.create_saml_provider"](
        name,
...
def saml_provider_absent(name, region=None, key=None, keyid=None, profile=None):
    ...
    if __opts__["test"]:
        ...
        ret["result"] = None
        return ret
    deleted = __salt__["boto3_iam.delete_saml_provider"](
        name, region=region, key=key, keyid=keyid, profile=profile
    )
