"""
Manage IAM roles using boto3.
=============================

    Renamed from ``boto_iam_role`` to ``boto3_iam_role`` and updated to call the
    refactored ``boto3_iam`` execution module.

Create and destroy IAM roles. Be aware that this interacts with Amazon's
...
def __virtual__():
    ...
    """
    if "boto3_iam.role_exists" in __salt__:
        return "boto3_iam_role"
    return (False, "boto3_iam module could not be loaded")
...
def _role_present(
    ...
    role = __salt__["boto3_iam.describe_role"](name, region, key, keyid, profile)
    if not role:
        if __opts__["test"]:
            ret["comment"] = f"IAM role {name} is set to be created."
            ret["result"] = None
            return ret
        created = __salt__["boto3_iam.create_role"](
        ...
        if not policy_document:
            _policy_document = __salt__["boto3_iam.build_policy"](region, key, keyid, profile)
        else:
        ...
        if salt.utils.dictdiffer.deep_diff(
            ...
            if __opts__["test"]:
                ...
                ret["result"] = None
                return ret
            updated = __salt__["boto3_iam.update_assume_role_policy"](
...
def _instance_profile_present(name, region=None, key=None, keyid=None, profile=None):
    ret = {"result": True, "comment": "", "changes": {}}
    exists = __salt__["boto3_iam.instance_profile_exists"](name, region, key, keyid, profile)
    if not exists:
        if __opts__["test"]:
            ret["comment"] = f"Instance profile {name} is set to be created."
            ret["result"] = None
            return ret
        created = __salt__["boto3_iam.create_instance_profile"](name, region, key, keyid, profile)
        if created:
...
def _instance_profile_associated(name, region=None, key=None, keyid=None, profile=None):
    ret = {"result": True, "comment": "", "changes": {}}
    is_associated = __salt__["boto3_iam.profile_associated"](
    ...
    if not is_associated:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        associated = __salt__["boto3_iam.associate_profile_to_role"](
            name, name, region, key, keyid, profile
        )
        if associated:
...
def _policies_present(
    ...
    for policy_name, policy in policies.items():
        _policy = __salt__["boto3_iam.get_role_policy"](
            name, policy_name, region, key, keyid, profile
        )
        if _policy != policy:
            policies_to_create[policy_name] = policy
    _list = __salt__["boto3_iam.list_role_policies"](name, region, key, keyid, profile)
    ...
    if policies_to_create or policies_to_delete:
        ...
        for policy_name, policy in policies_to_create.items():
            policy_set = __salt__["boto3_iam.create_role_policy"](
            ...
            if not policy_set:
                ...
                ret["result"] = False
                ret["comment"] = f"Failed to add policy {policy_name} to role {name}"
                return ret
        for policy_name in policies_to_delete:
            policy_unset = __salt__["boto3_iam.delete_role_policy"](
                name, policy_name, region, key, keyid, profile
            )
            if not policy_unset:
                _list = __salt__["boto3_iam.list_role_policies"](name, region, key, keyid, profile)
                ret["changes"]["new"] = {"policies": _list}
                ret["result"] = False
                ret["comment"] = f"Failed to remove policy {policy_name} from role {name}"
                return ret
        _list = __salt__["boto3_iam.list_role_policies"](name, region, key, keyid, profile)
        ret["changes"]["new"] = {"policies": _list}
...
def _policies_attached(
    ...
    for policy in managed_policies or []:
        ...
        for roledict in entities.get("policy_roles", []):
            if name == roledict.get("RoleName"):
                found = True
                break
        if not found:
            policies_to_attach.append(policy)
    _list = __salt__["boto3_iam.list_attached_role_policies"](
    ...
    if policies_to_attach or policies_to_detach:
        ...
        for policy_name in policies_to_attach:
            policy_set = __salt__["boto3_iam.attach_role_policy"](
                policy_name,
                role_name=name,
                region=region,
                key=key,
        ...
        for policy_name in policies_to_detach:
            policy_unset = __salt__["boto3_iam.detach_role_policy"](
                policy_name,
                role_name=name,
                region=region,
                key=key,
...
def _role_absent(name, region=None, key=None, keyid=None, profile=None):
    ...
    exists = __salt__["boto3_iam.role_exists"](name, region, key, keyid, profile)
    if exists:
        if __opts__["test"]:
            ret["comment"] = f"IAM role {name} is set to be removed."
            ret["result"] = None
            return ret
        deleted = __salt__["boto3_iam.delete_role"](name, region, key, keyid, profile)
        if deleted:
...
def _instance_profile_absent(name, region=None, key=None, keyid=None, profile=None):
    ...
    if exists:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        deleted = __salt__["boto3_iam.delete_instance_profile"](name, region, key, keyid, profile)
        if deleted:
...
def _instance_profile_disassociated(name, region=None, key=None, keyid=None, profile=None):
    ...
    if is_associated:
        if __opts__["test"]:
            ret["comment"] = f"Instance profile {name} is set to be disassociated."
            ret["result"] = None
            return ret
        associated = __salt__["boto3_iam.disassociate_profile_from_role"](
            name, name, region, key, keyid, profile
        )
        if associated:
            ...
            ret["result"] = False
            ret["comment"] = "Failed to disassociate {0} instance profile from {0} role.".format(
                name
            )
    return ret
