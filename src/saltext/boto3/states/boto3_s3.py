...
================================
    ...
    Renamed from ``boto_s3`` to ``boto3_s3`` and updated to call the
    refactored ``boto3_s3`` execution module.
    ...
    if HASH_METADATA_KEY in combined_extra_args["Metadata"]:
        ...
        if combined_extra_args["Metadata"][HASH_METADATA_KEY] != digest:
            ...
            ret["comment"] = (
                ...
                "do not pass it to the boto3_s3.object_present state.".format(HASH_METADATA_KEY)
            )
            return ret
